...
% the MGH-MF Waveform Database available on PhysioNet
...
% Reference for the MGH-MF Waveform Database:
% Welch JP, Ford PJ, Teplick RS, Rubsamen RM. The Massachusetts General
% Hospital-Marquette Foundation Hemodynamic and Electrocardiographic
% Database -- Comprehensive collection of critical care waveforms. J
...
for i = 50
    ...
    if i == 61 || i == 127 || i == 230 || i == 235 % These records have no annotations or no Matlab-readable data
        continue
    else
        ...
        % Find waveforms that correspond to ECGs
        ecg_leads = [];
        for k = 1:length(signal)
            text_indices = strfind(signal(k),'ECG');
            lead_text_indices = strfind(signal(k),'lead');
            if ~isempty(text_indices{1}) || ~isempty(lead_text_indices{1}) || strcmp(signal(k),'I') || strcmp(signal(k),'II') || strcmp(signal(k),'V')
                ecg_leads = [ecg_leads k];
            end
        end
        ...
        for p = 1:length(ecg_leads)
            ...
            clear filt_ECG segment segment_time
